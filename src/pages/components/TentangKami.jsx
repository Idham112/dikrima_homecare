...
const TentangKamiPage = () => {
	return (
		<div className="flex flex-col lg:flex-row ml-0 lg:ml-20">
			<div className="text-black basis-full lg:basis-2/3">
				<div className="flex flex-col gap-7 w-full lg:w-[700px] px-4 lg:px-0 text-center lg:text-left">
					<div>
						<h3 className="text-base text-[#238C7E]">{tentangKami.title}</h3>
					</div>
					<div>
						<h1 className="text-xl md:text-2xl lg:text-4xl text-[#3D5f5A]">
							{tentangKami.desc}
						</h1>
					</div>
					<div>
						<h3 className="text-base text-[#4A4A4A] w-full lg:w-[680px]">
							{tentangKami.cont}
						</h3>
					</div>
				...
				<div className="flex flex-col gap-6 lg:gap-12">
					<div className="flex flex-row gap-5">
						...
						<div className="flex flex-col justify-center w-full max-w-[310px]">
							<div className="text-sm text-[#121212]">{patient.number}</div>
							<div className="text-sm text-[#4A4A4A]">{patient.desc}</div>
						</div>
					</div>
					...
					<div className="flex flex-row gap-5">
						...
						<div className="flex flex-col justify-center w-full max-w-[310px]">
							<div className="text-sm text-[#121212]">{since.number}</div>
							<div className="text-sm text-[#4A4A4A]">{since.desc}</div>
						</div>
					</div>
				</div>
			</div>
